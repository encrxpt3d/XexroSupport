...
module.exports = {
  async execute(interaction) {
    const isThere = interaction.guild.channels.cache.find(
      c => c.topic && c.topic.match(interaction.user.id) && c.name.match("paid"))
    if (isThere) {
    ...
    const requestForm = new ModalBuilder()
      ...
      .setTitle('Request Paid MM Form')

    const requestReason = new TextInputBuilder()
      .setCustomId('request_reason')
      .setLabel('What do you need a paid middleman for?')
      .setPlaceholder('Paid Middleman Request Reason')
